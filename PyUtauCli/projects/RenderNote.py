...
class RenderNote:
    ...
    def __init__(
        self, note: Note, vb: VoiceBank, cachedir: str, output: str, mode2: bool = True
    ):
        """
        Parameters
        ----------
        ...
        if note.next is None:
            ...
            self._output_ms = (
                note.msLength
                + note.atPre.value
                + note.next.atOve.value
                - note.next.atPre.value
            )
    ...
    def _get_pitches(self, note: Note, mode2: bool) -> str:
        ...
        if mode2:
            ...
            if note.prev is not None and note.prev.lyric.value != "R":
                ...
                base_pitches += self._get_vibrato_pitches(
                    note.prev, t, offset - note.prev.msLength
                )
    ...
    def _get_base_pitches(self, note: Note, t: np.ndarray) -> np.ndarray:
        ...
        if note.next is not None and note.next.lyric.value != "R":
            ...
            if t[-1] >= note.next.pbs.time + next_offset:
                ...
                base_pitches[start:] = (
                    note.next.notenum.value - note.notenum.value
                ) * 100
        return base_pitches
    ...
    def _get_interp_param(
        self, x: np.ndarray, y: np.ndarray, t: np.ndarray, i: int
    ...
    def _get_interp_base(
        self, note: Note, offset: float
    ...
    def _get_vibrato_pitches(
        self, note: Note, t: np.ndarray, offset: float
    ) -> np.ndarray:
        """
        noteのpbs,pby,pbw,pbmとnote.prev.notenumを使ってピッチ列を返します。
        ...
        pitches[start : end + 1] = np.round(
            (
                np.sin(2 * np.pi / note.vibrato.cycle * phase + phase_offset)
    ...
    def _get_vibrato_fade(self, note: Note, t: np.ndarray) -> np.ndarray:
        ...
        fadeout_start_ms: float = (
            vibrato_ms - vibrato_ms * note.vibrato.fadeOutTime / 100
        )
        ...
        for i in range(len(t)):
            if t[i] <= fadeintime and fadeintime != 0:
                ...
                fade[i] = 1 - (
                    (t[i] - fadeout_start_ms) / (vibrato_ms - fadeout_start_ms)
                )
        return fade
    ...
    def encodeBase64(values: np.ndarray) -> list:
        ...
        for value in values:
            ...
            if value < 0:
                tmp += 4096
            result.append(
                RenderNote.encodeBase64Core(int(tmp / 64))
                + RenderNote.encodeBase64Core(tmp & 63)
            )
        return result
